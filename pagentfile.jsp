...
<%
	...
	try{
			...
			while(rs1.next()){
				count=rs1.getInt("agentfile")+1;
			
		String status="processing";
		try {
		
			Connection con1=databasecon.getconnection(); 
			PreparedStatement ps=con1.prepareStatement("update detect set agentfile=? where id='"+anomaly+"'");
				ps.setInt(1,count);
			...
			if(x!=0)
			{
				response.sendRedirect("agentfile.jsp?message=success");
			}
			else
			{
				response.sendRedirect("agentfile.jsp?message=fail");
			}
