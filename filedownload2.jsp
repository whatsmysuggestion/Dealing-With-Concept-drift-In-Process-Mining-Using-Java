...
<%
	...
	try{
			...
			while(rs1.next()){
				count=rs1.getInt("download")+1;
			
		String status="processing";
		try {
		
			Connection con1=databasecon.getconnection(); 
			PreparedStatement ps=con1.prepareStatement("update detect set download=? where id='"+anomaly+"'");
				ps.setInt(1,count);
			...
			if(x!=0)
			{
				response.sendRedirect("downloadleaker.jsp?message=success");
			}
			else
			{
				response.sendRedirect("downloadleaker.jsp?message=fail");
			}
